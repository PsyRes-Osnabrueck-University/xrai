...
e_net_pipeline = Pipeline(
    [("scaler", StandardScaler()), ("model", ElasticNet(random_state=42))]
)
...
mod_meta = GridSearchCV(
    estimator=svr_pipeline,
    param_grid=ensemble_params_grid,
    cv=5,
    scoring="r2",
    verbose=0,
    n_jobs=-1,
...
def find_params_xgb(
    ...
    if "xgb" in params:
        ...
        nrmse["xgb"].append(
            metrics.mean_squared_error(y_valid, xgb_pred, squared=False)
...
def find_params_rf(
    ...
    if "rf" in params:
        ...
        nrmse["rf"].append(
            metrics.mean_squared_error(y_valid, rf_pred, squared=False)
            / statistics.stdev(y_valid)
        )
    else:
...
def find_params_lasso(
    ...
    if not classed:
        mod_lasso = GridSearchCV(
            lasso_pipeline,
            {"model__alpha": np.arange(0.02, 2.0, 0.005)},
            cv=5,
            scoring="r2",
            verbose=0,
            n_jobs=-1,
    ...
    if "lasso" in params:
        ...
        nrmse["lasso"].append(
            metrics.mean_squared_error(y_valid, lasso_pred, squared=False)
            / statistics.stdev(y_valid)
        )
    else:
...
def find_params_e_net(
    ...
    if "e_net" in params:
        ...
        nrmse["e_net"].append(
            metrics.mean_squared_error(y_valid, e_net_pred, squared=False)
            / statistics.stdev(y_valid)
        )
    else:
...
def find_params_gpb(
    ...
    if random_effects:
        gp_model = gpb.GPModel(
            group_data=group_strain,
            group_rand_coef_data=X_strain[:, random_effects],
            ind_effect_group_rand_coef=[1] * len(random_effects),
            likelihood="gaussian",
        )
    else:
    ...
    data_train = gpb.Dataset(
        data=np.delete(X_strain, random_effects, axis=1), label=y_strain
    )
    opt_params = gpb.grid_search_tune_parameters(
        param_grid=gpb_param_grid,
        params=gpb_other_params,
        num_try_random=None,
        nfold=5,
        seed=1000,
        metric="rmse",
    ...
    pred_gpb = pred["response_mean"].reshape(
        -1,
    )
    y_valid = y_valid.reshape(
        -1,
    ...
    if "gpb" in params:
        ...
        nrmse["gpb"].append(
            metrics.mean_squared_error(y_valid, pred_gpb, squared=False)
            / statistics.stdev(y_valid)
        )
    else:
...
def find_params_svr(
    ...
    if "svr" in params:
        ...
        nrmse["svr"].append(
            metrics.mean_squared_error(y_valid, svr_pred, squared=False)
            / statistics.stdev(y_valid)
        )
    else:
...
def SuperLearner_fun(
    ...
    for model in run_list:
        ...
        if model == "gpb":
            ...
            if random_effects:
                pred = bst.predict(
                    data=np.delete(X_strain, random_effects, axis=1),
                    group_data_pred=group_strain,
                    group_rand_coef_data_pred=X_strain[:, random_effects],
                    predict_var=True,
                    pred_latent=False,
                )
            ...
            if random_effects:
                pred = bst.predict(
                    data=np.delete(X_valid, random_effects, axis=1),
                    group_data_pred=group_valid,
    ...
    ensemble_pred = ensemble_pred.reshape(
        -1,
    )
    ...
    if "super" in params:
        ...
        nrmse["super"].append(
            metrics.mean_squared_error(y_valid, ensemble_pred, squared=False)
            / statistics.stdev(y_valid)
        )
    else:
...
def cv_with_arrays(
    ...
    with open("cv_with_arrays.txt", "w") as f:
        # Redirect stdout to the file
        original_stdout = sys.stdout
        sys.stdout = f
        best_algorithms = []
        ...
        for col in range(df_cv.shape[1]):  # Jede spalte durchgehen
            ...
            for inner_fold in range(val_splits):  # jede Zeile durchgehen
                ...
                df_strain = df_ml[
                    (df_cv.iloc[:, col] != inner_fold) & (df_cv.iloc[:, col] != -1)
                ]  # X strain ist ungleich x_valid und ungleich x_test
                if not series_group.empty:
                    group_valid = series_group[
                        df_cv.iloc[:, col] == inner_fold
                    ]  # group_valid erstellen
                    group_strain = series_group[
                        (df_cv.iloc[:, col] != inner_fold) & (df_cv.iloc[:, col] != -1)
                    ]  # X strain ist ungleich x_valid und ungleich x_test
                ...
                X_strain_selected, X_valid_selected, y_strain, y_valid = (
                    X_strain.values,
                    X_valid.values,
                    y_strain.values,
                    y_valid.values,
                ...
                if "lasso" in run_list:
                    all_params_dict, r_dict, nrmse_dict = find_params_lasso(
                        X_valid=X_valid_selected,
                        X_strain=X_strain_selected,
                        y_valid=y_valid,
                        y_strain=y_strain,
                        ...
                        nrmse=nrmse_dict,
                        classed=classed,
                        group_strain=group_strain,
                    )
                if "gpb" in run_list and not series_group.empty:
                    all_params_dict, r_dict, nrmse_dict = find_params_gpb(
                        X_valid=X_valid_selected,
                        X_strain=X_strain_selected,
                        y_valid=y_valid,
                        y_strain=y_strain,
                        ...
                        group_strain=group_strain,
                        random_effects=Z_loc,
                        params=all_params_dict,
                        r=r_dict,
                        nrmse=nrmse_dict,
                    )
                if "merf" in run_list and not series_group.empty:
                    r_dict, nrmse_dict = find_params_merf(
                        X_valid=X_valid_selected,
                        X_strain=X_strain_selected,
                        y_valid=y_valid,
                        y_strain=y_strain,
                ...
                if "e_net" in run_list:
                    all_params_dict, r_dict, nrmse_dict = find_params_e_net(
                        X_valid=X_valid_selected,
                        X_strain=X_strain_selected,
                        y_valid=y_valid,
                        y_strain=y_strain,
                ...
                if "rf" in run_list:
                    all_params_dict, r_dict, nrmse_dict = find_params_rf(
                        X_valid=X_valid_selected,
                        X_strain=X_strain_selected,
                        y_valid=y_valid,
                        y_strain=y_strain,
                ...
                if "svr" in run_list:
                    all_params_dict, r_dict, nrmse_dict = find_params_svr(
                        X_valid=X_valid_selected,
                        X_strain=X_strain_selected,
                        y_valid=y_valid,
                        y_strain=y_strain,
                ...
                if "xgb" in run_list:
                    all_params_dict, r_dict, nrmse_dict = find_params_xgb(
                        X_valid=X_valid_selected,
                        X_strain=X_strain_selected,
                        y_valid=y_valid,
                        y_strain=y_strain,
                ...
                if "super" in run_list:
                    all_params_dict, r_dict, nrmse_dict = SuperLearner_fun(
                        X_valid=X_valid_selected,
                        X_strain=X_strain_selected,
                        y_valid=y_valid,
                        y_strain=y_strain,
            ...
            for (
                model_name
            ) in run_list:  # Alle rs und nrmses sammeln in jeweils einem df
                mean_r[model_name] = np.mean(r_dict[model_name][-val_splits:])
                z_r[model_name] = np.mean(r_dict[model_name][-val_splits:]) / np.std(
                    r_dict[model_name][-val_splits:]
                )
