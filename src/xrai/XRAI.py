...
from xrai.utils import (
    cor,
    identify_last_column,
    list_params,
    mean_ci,
    split_preparation,
)
...
class XRAI:
    ...
    def __init__(
        ...
        self.df = pd.read_excel(io=self.file_name)
    ...
    def load_algorithms(self, folder_path):
        # Load the best_algorithms.xlsx file into df_r
        self.best_algorithms = pd.read_excel("best_algorithms.xlsx").values.tolist()
        return True
    ...
    def fit(
        ...
        with open("fit.txt", "w") as f:
            ...
            with redirect_stdout(f):
                ...
                df_r.to_excel("r-inner-fold.xlsx")
                df_nrmse.to_excel("Nrmse-inner-fold.xlsx")
                pd.DataFrame(self.best_algorithms).to_excel("best_algorithms.xlsx")
        ...
        with open("transform_output.txt", "w") as f:
            ...
            with redirect_stdout(f):
                ...
                for i, col in enumerate(self.df_nested_cv.columns.tolist()):
                    ...
                    y_train_a = df_y_train.values.reshape(
                        -1,
                    )
                    y_test_a = df_y_test.values.reshape(
                        -1,
                    )
                    ...
                    if (
                        self.best_algorithms[i] == "e_net"
                        or self.best_algorithms[i] == "super"
                        and "e_net" in self.run_list
                    ):
                        ratios = np.arange(0.001, 0.3, 0.003)
                        alphas = [
                            0.5,
                    ...
                    if (
                        self.best_algorithms[i] == "lasso"
                        or self.best_algorithms[i] == "super"
                        and "lasso" in self.run_list
                    ):
                        if not self.classed_splits:
                    ...
                    if (
                        self.best_algorithms[i] == "xgb"
                        or self.best_algorithms[i] == "super"
                        and "xgb" in self.run_list
                    ):
                        xgb_params_grid = {
                            "max_depth": [2],
                            "learning_rate": [0.121],
                    ...
                    if (
                        self.best_algorithms[i] == "gpb"
                        or self.best_algorithms[i] == "super"
                        and "gpb" in self.run_list
                    ):
                        if self.Z_list:
                            gp_model = gpb.GPModel(
                                group_data=group_train,
                                group_rand_coef_data=Z_train_a,
                                ind_effect_group_rand_coef=[1] * len(self.Z_list),
                                likelihood="gaussian",
                            )
                        else:
                            gp_model = gpb.GPModel(
                    ...
                    if (
                        self.best_algorithms[i] == "merf"
                        or self.best_algorithms[i] == "super"
                        and "merf" in self.run_list
                    ):
                        z = np.array([1] * len(X_train_a)).reshape(-1, 1)
                        z = np.hstack([z, Z_train_a])
                        y_train_merf = y_train_a.reshape(
                            -1,
                        )
                    ...
                    nrmse_list.append(
                        metrics.mean_squared_error(y_test_a, pred, squared=False)
                        / statistics.stdev(y_test_a)
                    )
                    ...
                    if self.xAI == True:
                        if self.best_algorithms[i] == "super":
                            ...
                            explainer = shap.explainers.Permutation(
                                dict_models[i][self.best_algorithms[i]].predict,
                                masker=shap.sample(X_train_a, 100),
                                max_evals=2015,
                            )
                    ...
                    if "super" in self.best_algorithms:
                        ...
                        for i in range(start, len(shaps)):
                            ...
                            bs_values = np.concatenate(
                                (
                                    bs_values,
                                    np.array(shaps[i].base_values).reshape(
                                        -1,
                                    ),
                                ),
                                axis=0,
                ...
                if "super" in self.best_algorithms:
                    ...
                    outcome_dict["predicted"] = np.concatenate(
                        (outcome_dict["predicted"], outcome_dict["super_predicted"]),
                        axis=0,
                    )
                ...
                df_results_short = pd.DataFrame(
                    {
                        "Value": ["mean", "95% lower", "95% upper"],
                ...
                writer = pd.ExcelWriter(
                    os.path.join(self.output_path, "Results.xlsx"), engine="xlsxwriter"
                )
                # Write each dataframe to a different worksheet.
                ...
                if self.xAI == True:
                    ...
                    df_shap_values = pd.DataFrame(
                        global_shap_values.reshape(-1, len(global_shap_values)),
                    ...
                    df_shap_values_new = pd.DataFrame(
                        {
                            ...
                            "SHAP-value": df_shap_values.iloc[0].tolist(),
                        }
                    )
                    df_shap_values_new["percent_shap_value"] = (
                        df_shap_values_new["SHAP-value"]
                        / df_shap_values_new["SHAP-value"].sum()
                        * 100
                    )
                    dict_cors = {}
                    n_rows = [
                        len(df_id_data.loc[df_id_data["fold"] == fold, "fold"])
    ...
    def save_plots(self):
        ...
        plt.savefig(os.path.join(self.output_path, "waterfall_plot.png"))
        # plt.show()
        shap.plots.heatmap(self.shap_values)
        plt.savefig(os.path.join(self.output_path, "heatmap.png"))
        # plt.show()
        return 0
