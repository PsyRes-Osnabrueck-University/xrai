...
def find_params_svr(X_valid, X_strain, y_valid, y_strain, svc_params, svc_f1):
...
def SuperLearner_fun(X_valid, X_strain, y_valid, y_strain, super_f1, xgb_params, lasso_params,
                 rf_params,svr_params):
    params_dict = {}
    all_params = {"lasso": lasso_params, "xgb": xgb_params, "rf": rf_params, "svc": svr_params}
    ...
    dict_models = {"xgb": mod_xgb, "lasso": mod_lasso, "rf": mod_rf, "svc": mod_svc}
...
def cv_with_arrays(df_ml_to_array, df_cv_to_array, val_splits, run_list):
    ...
    for col in range(array_cv.shape[1]):  # Jede spalte durchgehen
        ...
        for inner_fold in range(val_splits):  # jede Zeile durchgehen
            ...
            if "svc" in run_list: svc_f1, svc_params = find_params_svr(X_valid=X_valid, X_strain=X_strain, y_valid=y_valid, y_strain=y_strain, svc_params=svc_params, svc_f1=svc_f1)
            ...
            if "super" in run_list: super_f1 = SuperLearner_fun(X_valid=X_valid, X_strain=X_strain, y_valid=y_valid, y_strain=y_strain, super_f1=super_f1, xgb_params=xgb_params, lasso_params=lasso_params, rf_params=rf_params, svc_params=svc_params)

        f1s = {"xgb": xgb_f1[-val_splits:], "rf": rf_f1[-val_splits:],
               "svr": svc_f1[-val_splits:], "super": super_f1[-val_splits:]}
        mean_nrmse = {"xgb": np.mean(xgb_f1[-val_splits:]), "rf": np.mean(rf_f1[-val_splits:]),
               "svr": np.mean(svc_f1[-val_splits:]), "super": np.mean(super_f1[-val_splits:])}
        best_algorithms.append(sorted(mean_nrmse, key=lambda key: mean_nrmse[key])[-1])
    ...
    r2s = {"xgb": xgb_f1, "rf": rf_f1, "svc": svc_f1, "super": super_f1}
    all_params = {"xgb": xgb_params, "rf": rf_params, "svc": svc_params}

    for model_name in run_list:                  # Alle R2s und nrmses sammeln in jeweils einem df
        df_f1[model_name] = f1s[model_name]

    return df_f1, all_params, lasso_params, xgb_params, rf_params, svc_params, best_algorithms
...
# Ensemble:
xgb_ensemble_grid = {'xgb__max_depth': [2],
...
xgb_meta_pipeline = Pipeline(
    steps=[
        ("labelbin", LabelBinarizer()),
        ("xgb", xgbc)
    ]
)
mod_ensemble = GridSearchCV(estimator=xgb_meta_pipeline, param_grid=xgb_ensemble_grid, scoring='f1_weighted', verbose=0, n_jobs=-1, cv=5)


# Bei fertigem Datansatz AB HIER!!! --------------------------------------------------------------------------------------------------
outcome = "diagnosen"  # Alternativ "srs_ges"
...
df_f1, all_params, lasso_params, xgb_params, rf_params, svr_params, best_algorithms = cv_with_arrays(
...
for i, col in enumerate(df_nested_cv.columns.tolist()):  # Jede spalte durchgehen
    ...
    if best_algorithms[i]=="svc" or best_algorithms[i]=="super":
        ...
        mod_svr = GridSearchCV(estimator=svc_pipeline, param_grid=svc_pars, scoring="f1_weighted",
